...
function toggleIngredient(ing, chipEl){
  if(selected.has(ing)){
    selected.delete(ing);
...
chipGridWrap.addEventListener('click', (e) => {
  const chip = e.target.closest('.chip');
  if(!chip) return;
  toggleIngredient(chip.dataset.ing, chip);
});
...
document.getElementById('clear-btn').addEventListener('click', () => {
  ...
  chipGridWrap.querySelectorAll('.chip').forEach(c => c.classList.remove('active'));
  renderCustomRow();
  renderResults();
});
...
function runSearch(){
  ...
  const allChips = chipGridWrap.querySelectorAll('.chip');
  let anyVisible = false;
  allChips.forEach(chip => {
    const name = chip.dataset.ing.toLowerCase();
    const match = !q || name.includes(q);
    chip.classList.toggle('hidden-by-search', !match);
    if(match) anyVisible = true;
  });
  chipGridWrap.querySelectorAll('.ing-cat-group').forEach(group => {
    const visibleChips = group.querySelectorAll('.chip:not(.hidden-by-search)');
    group.style.display = (q && visibleChips.length === 0) ? 'none' : '';
  });
  if(q && !anyVisible){
    noResultAdd.classList.add('show');
    noResultAddLabel.textContent = q;
  } else {
    noResultAdd.classList.remove('show');
  }
}
searchInput.addEventListener('input', runSearch);

noResultAddBtn.addEventListener('click', () => {
...
function addCustomIngredient(name){
  if(!name || customIngredients.has(name)) return;
...
function renderCustomRow(){
  ...
  customRow.innerHTML = Array.from(customIngredients).map(name => `
    <span class="custom-chip" data-name="${name}">${name}<button type="button" aria-label="삭제">×</button></span>
  `).join('');
}
...
customRow.addEventListener('click', (e) => {
  ...
  if(!btn) return;
  const chip = btn.closest('.custom-chip');
  const name = chip.dataset.name;
  customIngredients.delete(name);
